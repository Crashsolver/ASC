...
class App(tk.Tk):
    ...
    def set_gui(self):
        ...
        self.result_text.configure(background="#d8f8d8", wrap="word", height=38, width=80,fg="#000000")
    ...
    def get_lookup_data(self):
        ...
        if record_check != None and record_check != []:
            if len(record_check) == 1:
                ...
                ## more than one match, place in Text widget
                ## first blank out any previous result
                self.their_call_var.set('')
                self.their_county_var.set('')
                self.their_state_var.set('')
                self.their_session_count_var.set('')
